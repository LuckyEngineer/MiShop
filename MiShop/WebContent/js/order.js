	...
	$.ajax({
		...
		success:function(result){
			...
			for(var j=0;j<data.length;j++){
				var name="info"+i;
				 var str = '<li class="one-address '+name+'" data-id="'+data[j].addressId+'">'+
			        '<h4 class="user" style="display:block">'+data[j].receiver+'</h4>'+
			        '<p class="mobile" style="display:block">'+data[j].tel+'</p>'+
			        '<p class="city" style="display:block">'+data[j].province+"&nbsp;"+data[j].city+'</p>'+
			        '<p class="small_city" style="display:block">'+data[j].address+'</p>'+
			        '<div class="query-address" style="display:block">'+
			        '<a href="javaScript:void(0)" class="d-query" style="display:block">修改</a>'+
			        '</div>'+
			        '</li>';
			    $(".a-content").prepend(str);
			}
		}
	});
})();
...
//提交订单
$(".c_r_make").click(function(){
	var arr=[];
	var priceAll = 0;
	$(".content_centre2").each(function () {
		var orderObject={};
		var info = $(this).find(".c_name").html();
		var productId = $(this).attr("product-id");
		var productImage = $(this).find(".content_centre2_msg2").attr("src");
		var productName = info.split("&nbsp;")[0];
		var config = info.split("&nbsp;")[1];
		var color = info.split("&nbsp;")[2];
		var unitPrice = $(this).find(".c_money_price").html();
		var quantity = $(this).find(".c_number").html();
		var totalPrice = $(this).find(".c_count_totalPrice").html();
        ...
        priceAll = priceAll + parseInt(totalPrice);
    });
	var jsonArray=JSON.stringify(arr);
	alert(addr);
    $.ajax({
    	url:"OrderCtrl?method=addOrder",
    	type:"post",
    	data:{
    		"jsonArray":jsonArray,
    		"priceAll":priceAll,
